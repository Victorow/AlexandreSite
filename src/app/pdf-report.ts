...
const C = {
  ...
  primaryDark: [30, 58, 138] as [number, number, number],// blue-900
...
export function generateAssessmentPDF(data: AssessmentPdfData): jsPDF {
  ...
  // ---- garante espaço; cria página nova se necessário ----
  ...
  const runningHeader = () => {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(student.name, PAGE_W - M, M, { align: 'right' });
    setDraw(C.line);
  ...
  const sectionTitle = (label: string) => {
    ...
    setFill(C.primary);
    doc.rect(M, y, 3, 6, 'F');               // acento vertical
    setText(C.ink);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
  ...
  setFill(C.cardBg);
  doc.roundedRect(M, y, CONTENT_W, 26, 2, 2, 'F');

  const colW = CONTENT_W / 4;
  const infoCell = (i: number, row: number, label: string, value: string) => {
  ...
  infoCell(0, 0, 'Aluno', student.name.length > 22 ? student.name.slice(0, 21) + '…' : student.name);
  infoCell(1, 0, 'Idade', `${age} anos`);
  infoCell(2, 0, 'Sexo', pdfGenderLabel(student.gender));
  infoCell(3, 0, 'Altura', `${student.height_cm} cm`);
  infoCell(0, 1, 'Data da Avaliação', pdfFormatDate(a.date));
  infoCell(1, 1, 'Avaliação Anterior', prev ? pdfFormatDate(prev.date) : '—');
  infoCell(2, 1, 'Objetivo', (student.goal || '—').length > 20 ? student.goal.slice(0, 19) + '…' : (student.goal || '—'));
  infoCell(3, 1, 'Telefone', student.phone_number || '—');
  y += 32;
  ...
  interface MetricCard {
    ...
    value: string;
    unit: string;
    sub?: string;          // classificação
    delta?: DeltaInfo;
    improve?: 'down' | 'up' | 'neutral';
  }
  ...
  const drawCard = (idx: number, m: MetricCard) => {
    ...
    if (m.sub) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.5);
      doc.text(m.sub, subX, cy + 20);
      subX += doc.getTextWidth(m.sub) + 3;
    }
    if (m.delta && m.delta.text !== '—' && m.delta.dir !== 'flat') {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7.5);
      doc.text(`${m.delta.text} vs. ant.`, subX, cy + 20);
    }
  };
  ...
  const cards: MetricCard[] = [
    {
      label: 'Peso', value: pdfFormatNumber(bio?.weight_kg), unit: 'kg',
      delta: pdfDelta(bio?.weight_kg, pbio?.weight_kg), improve: 'neutral',
    },
    {
      label: 'IMC', value: pdfFormatNumber(a.bmi), unit: '',
      sub: a.bmi_classification, delta: pdfDelta(a.bmi, prev?.bmi), improve: 'neutral',
    },
    {
      label: '% Gordura', value: pdfFormatNumber(a.body_fat_percentage), unit: '%',
      sub: a.body_fat_classification,
      delta: pdfDelta(a.body_fat_percentage, prev?.body_fat_percentage), improve: 'down',
    },
    {
      label: 'Massa Magra', value: pdfFormatNumber(a.lean_mass_kg), unit: 'kg',
      delta: pdfDelta(a.lean_mass_kg, prev?.lean_mass_kg), improve: 'up',
    },
    {
      label: 'Idade Corporal', value: pdfFormatNumber(bio?.body_age, 0), unit: 'anos',
      delta: pdfDelta(bio?.body_age, pbio?.body_age, 0), improve: 'down',
    },
    {
      label: 'Gordura Visceral', value: pdfFormatNumber(bio?.visceral_fat_level, 0), unit: '',
      sub: pdfVisceralLabel(a.visceral_risk),
      delta: pdfDelta(bio?.visceral_fat_level, pbio?.visceral_fat_level, 0), improve: 'down',
    },
  ];
  cards.forEach((m, i) => drawCard(i, m));
  y += cardH * 2 + cardGap + 6;

  // ---- tabela genérica comparativa ----
  ...
  const drawTable = (title: string, rows: Row[], showCompare: boolean) => {
    ...
    // colunas
    const cLabel = M + 3;
    const cCur = showCompare ? M + CONTENT_W * 0.48 : M + CONTENT_W * 0.7;
    const cPrev = M + CONTENT_W * 0.68;
    ...
    rows.forEach((r, i) => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.text(r.label, cLabel, y + 4.7);
      doc.setFont('helvetica', 'bold');
      doc.text(r.cur, cCur, y + 4.7);
      if (showCompare) {
        doc.setFont('helvetica', 'normal');
        setText(C.muted);
  ...
  const cRow = (label: string, key: keyof typeof cir): Row => ({
    label,
    cur: pdfFormatNumber(cir?.[key] as number),
    prevVal: pdfFormatNumber(pcir?.[key] as number),
    delta: pdfDelta(cir?.[key] as number, pcir?.[key] as number),
  });
  ...
  // ---- Anamnese (se houver) ----
  if (student.anamneses) {
    const an = student.anamneses;
    const yesNo = (b: boolean) => (b ? 'Sim' : 'Não');
    drawTable('Anamnese / Histórico de Saúde', [
      { label: 'Condição cardíaca', cur: yesNo(an.cardiac_condition) },
      { label: 'Dor articular', cur: yesNo(an.joint_pain) },
      { label: 'Dor no peito ao exercitar', cur: yesNo(an.chest_pain_during_exercise) },
      { label: 'Cirurgia recente', cur: an.recent_surgery_description || '—' },
      { label: 'Medicações em uso', cur: an.active_medications || '—' },
      { label: 'Observações', cur: an.notes || '—' },
    ], false);
  }
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.text(`Personal Trainer: ${trainerName}`, M, FOOTER_Y);
